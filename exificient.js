...
function StringTable() {
	...
	StringTable.prototype.getLocalValue = function(qnameID, localValueID){
		for(var i=localValueID; i<this.strings.length; i++) {
			if(this.strings[i].qnameID == qnameID && this.strings[i].localValueID == localValueID) {
	...
	StringTable.prototype.addValue = function(qnameID, value){
		...
		this.strings.push(new StringTableEntry(qnameID, value, globalValueID, localValueID));
	}
...
function AbtractEXICoder(grammars) {
	...
	AbtractEXICoder.prototype.getCodeLengthForGrammar = function(grammar){
		...
		} else if (grammar.type == "docEnd" || grammar.type == "fragmentContent") {
			// TODO CM, PI
			return 0; 
		} else if (grammar.type == "firstStartTagContent") {
			if(this.isStrict) {
				return this.getCodeLength(grammar.production.length + ( (grammar.isTypeCastable || grammar.isNillable ) ? 1 : 0 ));
			} else {
		...
		if(nbits < 0) {
			...
			if(this.errn == 0) {
				if(nbits <= this.capacity) {
					...
					var b = (this.buffer >> this.capacity) & (0xff >> (8 - nbits));
					return b;
...
function EXIDecoder(arrayBuffer, grammars) {
	...
	EXIDecoder.prototype.decodeHeader = function(){
		// TODO cookie
		var distBits = this.bitStream.decodeNBitUnsignedInteger(2); // Distinguishing Bits
		if(distBits != 2) {
			alert("Error: Distinguishing Bits are " + distBits);
			return -1;
		}
		var presBit = this.bitStream.decodeNBitUnsignedInteger(1); // Presence Bit for EXI Options
		if(presBit != 0) {
			alert("Error: Do not support EXI Options in header");
			return -1;
		}
		// TODO continuos  e.g., Final version 16 == 0 1111 0000	 
		var formatVersion = this.bitStream.decodeNBitUnsignedInteger(5);  // Format Version
		if(formatVersion != 0) {
	...
	EXIDecoder.prototype.decodeDatatypeValue = function(datatype, qnameID, qnameContext){
		...
		if(datatype.type == "STRING") {
			...
			case 0:
				/* local value hit*/
				var n = this.getCodeLength(this.stringTable.getNumberOfLocalStrings(qnameID));
			...
			case 1:
				/* global value hit*/
				var n = this.getCodeLength(this.stringTable.getNumberOfGlobalStrings());
			...
			for(i=0; i<this.eventHandler.length; i++) {
				...
				if(qnameContext == null || qnameContext == undefined) {
					...
					eh.attribute("", qnameContext.localName, mantissa + "E" + exponent); // TODO uri
				}
			}	
		} else if(datatype.type == "BOOLEAN") {
			var b = this.bitStream.decodeNBitUnsignedInteger(1) == 0 ? false : true;
			...
			var presenceTimezone = this.bitStream.decodeNBitUnsignedInteger(1) == 0 ? false : true;
			...
			for(var i=0; i<this.eventHandler.length; i++) {
				...
				if(qnameContext == null || qnameContext == undefined) {
					...
					eh.attribute("", qnameContext.localName, sDatetime); // TODO uri
				}
			}
	...
	EXIDecoder.prototype.decodeElementContext = function(grammar, elementQNameID){
		...
		while(!popStack) {
			...
			case "endDocument":
				...
				break;
			case "startElement":
				// console.log("\t" + "StartElement qnameID " + prod.startElementQNameID );
				// console.log("\t" + "StartElement name " + getQNameContext(prod.startElementQNameID).localName);
				
				var seGrammar = grammars.grs.grammar[prod.startElementGrammarID];
				var qnameContext = this.getQNameContext(prod.startElementQNameID);
			...
			case "endElement":
				...
				break;
			case "attribute":
				// console.log("\t" + "Attribute qnameID " + prod.attributeQNameID );
				// console.log("\t" + "Attribute name " + getQNameContext(prod.attributeQNameID).localName);
				// console.log("\t" + "Attribute datatypeID " + prod.attributeDatatypeID );
				...
				this.decodeDatatypeValue(datatype, prod.attributeQNameID, qnameContextA);
				
				break;
			case "characters":
				// console.log("\t" + "Characters datatypeID " + prod.charactersDatatypeID );
	...
	EXIDecoder.prototype.decode = function(){
		...
		console.log("\t" + "Number of NamespaceContexts" + Object.keys(grammars.qnames.namespaceContext).length);
		...
		if(errn == 0) {
			...
			// Document grammar
			console.log("\t" + "number of grammars: " + grammars.grs.grammar.length);
			console.log("\t" + "Document grammar ID: " + grammars.grs.documentGrammarID);
...
function BitOutputStream(){
	...
	BitOutputStream.prototype.encodeNBitUnsignedInteger = function(b, n){
		if (n == 0) {
			...
			this.buffer = b; // Note: the high bits will be shifted out during
			// further filling
			this.capacity = 8 - n;
		}
	}
	...
	/**
	 * Encode a string as a sequence of codepoints, each of which is encoded
...
function EXIEncoder(grammars) {
	...
	EXIEncoder.prototype.encodeXmlText = function(textXML){
		/* should allow parsing XML string into an XML document in all major browsers, including Internet Explorer 6. */
		var parseXml;
		...
		if(el.attributes != null && el.attributes.length > 0) {
			if(el.attributes.length > 1) {
				...
				for (var i=0; i<el.attributes.length; i++) {
					// console.log("  AT " + el.attributes[i].nodeName + " == " + el.attributes[i].nodeValue);
				...
				for(var i=0; i<atts.length; i++) {
					...
					if(at != null) {
						...
						// when does this happen, only for schemaLocations and such?
					}
				}
			} else {
				this.attribute(el.attributes[0].namespaceURI, el.attributes[0].localName, el.attributes[0].nodeValue);
			}
		}
	...
	EXIEncoder.prototype.startElement = function(namespace, localName) {
		...
		if(ec != -1) {
			...
			this.elementContext.push(new ElementContextEntry(prod.startElementQNameID, startElementGrammar));
		} else {
		...
		if("http://www.w3.org/2000/xmlns/" == namespace) {
			...
			for(var i=0; ec == -1 && i<grammar.production.length; i++) {
				...
				if(prod.event == "attribute") {
					var qnameContext = this.getQNameContext(prod.attributeQNameID);
					if(qnameContext.localName == localName) {
						// TODO namespace
						ec = i;	
					}
			...
			if(ec != -1) {
				...
				// write value
				var datatype = this.grammars.simpleDatatypes[prod.attributeDatatypeID];
				this.encodeDatatypeValue(value, datatype, prod.attributeQNameID);
	...
	EXIEncoder.prototype.characters = function(chars){
		...
		if(ec != -1) {
			...
			// write value
			var datatype = this.grammars.simpleDatatypes[prod.charactersDatatypeID];
			this.encodeDatatypeValue(chars, datatype, this.elementContext[this.elementContext.length-1].qnameID);
			// update current element context with revised grammar
			var nextGrammar =  grammars.grs.grammar[prod.nextGrammarID];
			this.elementContext[this.elementContext.length-1].grammar = nextGrammar;
		} else {
	...
	/*function parseMonth(sb, pos, dateTimeValue) {
	...
	/*
	function parseDay(sb, pos) {
		var sDay = sb.substring(pos, pos+2);
	...
	EXIEncoder.prototype.encodeDatatypeValue = function(value, datatype, qnameID){
		if(datatype.type == "STRING") {
			...
			if(stEntry == null) {
				...
				if(stEntry.qnameID == qnameID) {
					...
					var n = this.getCodeLength(this.stringTable.getNumberOfLocalStrings(qnameID));
					this.bitStream.encodeNBitUnsignedInteger(stEntry.localValueID, n);
				} else {
					// global hit
					this.bitStream.encodeUnsignedInteger(1);
					var n = this.getCodeLength(this.stringTable.getNumberOfGlobalStrings());
					this.bitStream.encodeNBitUnsignedInteger(stEntry.globalValueID, n);
				}
			}		
		} else if(datatype.type == "FLOAT") {
			...
			console.log("\t" + " floatB = " + fl.mantissa + " E " + fl.exponent);
		} else if(datatype.type == "BOOLEAN") {
			...
			var sDatetime = "";
			if(datatype.datetimeType == "date") { //  // date: Year, MonthDay, [TimeZone]
				// YEAR_OFFSET = 2000
				// NUMBER_BITS_MONTHDAY = 9
				// MONTH_MULTIPLICATOR = 32
				...
				this.bitStream.encodeNBitUnsignedInteger(dateTimeValue.monthDay, 9);
			} else {
				alert("Unsupported datetime type: " + datatype.datetimeType);
